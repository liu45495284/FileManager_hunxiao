...
public class FileIconLoader implements Callback {
    ...
    private class LoaderThread extends HandlerThread implements Callback {
        ...
        private Bitmap getImageThumbnail(long id) {
            Bitmap thumbnail = Images.Thumbnails.getThumbnail(mContext.getContentResolver(), id, MICRO_KIND, null);
            if(thumbnail != null){
                thumbnail = Util.toRoundCorner(thumbnail, 10);
            }
            return thumbnail;
        }

        private Bitmap getVideoThumbnail(long id) {
            Bitmap thumbnail = Video.Thumbnails.getThumbnail(mContext.getContentResolver(), id, MICRO_KIND, null);
            if(thumbnail != null){
                thumbnail = Util.toRoundCorner(thumbnail, 10);
            }
            return thumbnail;
