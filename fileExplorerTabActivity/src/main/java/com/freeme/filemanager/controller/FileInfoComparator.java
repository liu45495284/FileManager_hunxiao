...
public class FileInfoComparator implements Comparator<FileInfo>, Serializable {
    ...
    private int sortByType(FileInfo op, FileInfo oq) {
        if (op.isDirectory() && !oq.isDirectory()) {
            ...
            if (FileManagerOperationActivity.sCategoryFoldersPath != null
                    && !FileManagerOperationActivity.sCategoryFoldersPath.isEmpty()
                    && (sequenceSearch(FileManagerOperationActivity.sCategoryFoldersPath, op
                            .getFilePath()) >= 0)
                    && (sequenceSearch(FileManagerOperationActivity.sCategoryFoldersPath, oq
                            .getFilePath()) < 0)) {
                return -1;
            } else if (FileManagerOperationActivity.sCategoryFoldersPath != null
                    && !FileManagerOperationActivity.sCategoryFoldersPath.isEmpty()
                    && (sequenceSearch(FileManagerOperationActivity.sCategoryFoldersPath, op
                            .getFilePath()) < 0)
                    && (sequenceSearch(FileManagerOperationActivity.sCategoryFoldersPath, oq
     ...
     */
    protected static int binarySearch(String[] array, String key) {
