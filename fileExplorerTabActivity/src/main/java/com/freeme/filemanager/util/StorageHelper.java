...
//                  newMountVolumeList.set(2, storageVolume);
    ...
    public boolean isVolumeMounted(String path) {
        ...
        if(this.mStorageManager != null && !TextUtils.isEmpty(path)){
        ...
        UsbStrogeInfo usbStrogeInfo=null;
        ...
        if(path.startsWith(Util.USBOTG_DIR)){
             usbStrogeInfo = Util.getUsbStorgeInfo();
            if(usbStrogeInfo!=null&&usbStrogeInfo.total>0&&usbStrogeInfo.free!=usbStrogeInfo.total){
                return true;
            }
        }
